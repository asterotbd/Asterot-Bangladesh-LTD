...
export default function LoginForm({ mode = 'user' }: { mode?: LoginMode }) {
  return (
    <Suspense>
...
function LoginFormInner({ mode }: { mode: LoginMode }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (error) setError(error.message)
      ...
      // Server-side role verification + redirect: never trust client-side checks.
      window.location.assign('/api/auth/admin-entry')
    } else {
      const next = searchParams.get('next')
      if (isSafeInternalPath(next)) router.push(next as string)
